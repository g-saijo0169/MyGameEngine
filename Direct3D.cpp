...
//スワップチェーンからバックバッファを取得（バックバッファ ＝ レンダーターゲット）
    ...
    {
        //エラー処理
        MessageBox(nullptr, "スワップチェーンからバックバッファの取得に失敗しました", "エラー", MB_OK);
        return hr;
    }
...
HRESULT Direct3D::InitShader2D()
{
   ...
   hr = pDevice_->CreateInputLayout(layout, 2, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &(shaderBundle[SHADER_3D].pVertexLayout_));
   if (FAILED(hr))
   {
       //エラー処理
       MessageBox(nullptr, "頂点インプットレイアウトの作成に失敗しました", "エラー", MB_OK);
...
void Direct3D::SetShader(SHADER_TYPE type)
{
    ...
    pContext_->IASetShader(shaderBundle[type].pVertexLayout_);
    pContext_->RSSetShader(shaderBundle[type].pRastaerizerState_);
}
